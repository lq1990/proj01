...
<div class="logintop">    
    ...
    <%--
    	...
    	Integer flag = (Integer) request.getAttribute("flag"); // 0
    	if(flag != null && flag == 0) {
    		info = "用户名或密码错误";
    	}
    	
    	Integer pwd = (Integer) session.getAttribute("pwd"); // 1
    	if(pwd != null && pwd == 1) {
    		info = "密码修改成功";
    		color = "green";
    ...
    <form action="user" method="post" >
	    ...
	    <c:choose>
	    	<c:when test="${flag == 0 }">
			     <span style='font-size: 12px; font-weight: bold; color: red;'>jstl 用户名或密码错误</span>
			     <c:remove var="flag" />
	    	</c:when>
	    	<c:when test="${pwd == 1 }">
			     <span style='font-size: 12px; font-weight: bold; color: green;'>jstl 密码修改成功</span>
			     <c:remove var="pwd" />
	    	</c:when>
	    	<c:when test="${reg == 2 }">
			     <span style='font-size: 12px; font-weight: bold; color: green;'>jstl 注册成功，请登录</span>
			     <c:remove var="reg" />
	    	</c:when>
	    </c:choose>
